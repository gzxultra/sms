...
class Query(object):
    ...
    def fields(self, *fields):
        ...
        self.type = 'SELECT ' + (','.join('`%s`' % f for f in fields))
        return self

    def ids(self):
        self.mapper = lambda x: x[0]
        self.type = 'SELECT id'
        return self
    ...
    def use_index(self, name, force=False):
        if force:
            self.index = 'force index (%s)' % name
        else:
            self.index = 'use index (%s)' % name
